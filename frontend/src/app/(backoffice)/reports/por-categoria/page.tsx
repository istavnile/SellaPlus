...
export default function PorCategoriaPage() {
  ...
  const exportCsv = () => {
    const headers = 'Categoría,Artículos vendidos,Ventas netas,Costo de bienes,Beneficio bruto\n';
    const rows = data.map((r) =>
      `${r.name},${Number(r.itemsSold).toFixed(0)},${r.netSales.toFixed(2)},${r.cogs.toFixed(2)},${r.grossProfit.toFixed(2)}`,
    ).join('\n');
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs font-bold text-gray-500 border-b border-gray-200 bg-[#fbfbfb]">
                      ...
                      <th className="px-5 py-4 font-bold text-center">Artículos vendidos</th>
                      <th className="px-5 py-4 font-bold text-center">Ventas netas</th>
                      <th className="px-5 py-4 font-bold text-center">Cost of goods</th>
                      <th className="px-5 py-4 font-bold text-center">Beneficio bruto</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.map((r) => (
                      <tr key={r.categoryId} className="border-b border-gray-100 hover:bg-gray-50 text-gray-800">
                        <td className="px-5 py-4 font-medium border-r border-gray-50">{r.name}</td>
                        <td className="px-5 py-4 text-center font-medium">{Number(r.itemsSold).toFixed(0)}</td>
                        <td className="px-5 py-4 text-center font-medium">{money(r.netSales)}</td>
                        <td className="px-5 py-4 text-center text-gray-500">{money(r.cogs)}</td>
                        <td className="px-5 py-4 text-center font-medium">{money(r.grossProfit)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

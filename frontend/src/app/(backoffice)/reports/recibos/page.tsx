  ...
  const exportCsv = () => {
    ...
    const rows = data.map((r) => {
      const vals = activeCols.map(c => {
        ...
        if (c.id === 'items') return `"${r.items?.map(i => `${Number(i.quantity)} x ${i.productName}`).join(', ') ?? ''}"`;
                ...
                {loading ? (
                  ...
                  >
                    ...
                    {visibleColumns.includes('items') && (
                      <td className="px-5 py-4 text-gray-600 truncate max-w-[200px]" title={r.items?.map(i => `${Number(i.quantity)} x ${i.productName}`).join(', ')}>
                        {r.items?.map(i => `${Number(i.quantity)} x ${i.productName}`).join(', ') || '—'}
                      </td>
                    )}

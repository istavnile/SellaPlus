...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get(`/products/${id}`)
      ...
      .catch(() => { toast.error('No se pudo cargar el producto'); router.push('/products'); })
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div>
            ...
            <input {...register('name')} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name.message}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
            <textarea {...register('description')} rows={3} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Categoría</label>
            <select {...register('categoryId')} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('sku')} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Código de barras</label>
              <input {...register('barcode')} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input {...register('costPrice')} type="number" step="0.01" min="0" className="w-full border border-gray-300 rounded-lg pl-7 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-2.5 text-gray-400 text-sm">{symbol}</span>
                <input {...register('basePrice')} type="number" step="0.01" min="0" className="w-full border border-gray-300 rounded-lg pl-7 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          {trackStock && (
            <div>
              ...
              <input {...register('stockAlertThreshold')} type="number" min="0" className="w-40 border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <p className="text-sm text-gray-500">Agrega nuevas variantes aquí. Las variantes existentes no se modificarán.</p>
          ...
          {optionFields.map((field, idx) => (
            <div key={field.id} className="border border-gray-200 rounded-lg p-4 space-y-3">
              ...
              <input {...register(`options.${idx}.name`)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="Nombre (ej: Talla)" />
              <input {...register(`options.${idx}.values`)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="Valores separados por coma (ej: S, M, L)" />
            </div>
          ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {POS_COLORS.map((color) => (
                    <button key={color} type="button" onClick={() => setValue('posColor', color)}
                      className={cn('w-11 h-11 rounded-md transition-all', posColor === color && 'ring-2 ring-offset-2 ring-gray-500')}
                      style={{ backgroundColor: color }}>
                      {posColor === color && <span className="flex items-center justify-center text-white text-lg font-bold">✓</span>}
                    </button>
                  ))}
                </div>
                <div className="flex gap-2">
                  {POS_SHAPES.map(({ id: shapeId }) => (
                    <button key={shapeId} type="button" onClick={() => setValue('posShape', shapeId)}
                      className={cn('w-11 h-11 flex items-center justify-center rounded-md border-2 transition-all', posShape === shapeId ? 'border-gray-500' : 'border-gray-200 hover:border-gray-300')}>
                      <ShapePreview shape={shapeId} color={posShape === shapeId ? '#374151' : '#d1d5db'} size={28} />
                    </button>
                  ))}
                </div>
              </div>

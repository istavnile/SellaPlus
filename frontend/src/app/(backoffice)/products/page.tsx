...
import { ProductDrawer } from '@/components/backoffice/product-drawer';
...
export default function ProductsPage() {
  ...
  const openCreate = () => { setDrawerProductId(null); setDrawerOpen(true); };
  const openEdit   = (id: string) => { setDrawerProductId(id); setDrawerOpen(true); };
  ...
  const load = useCallback(() => {
    ...
    const params: any = { page, limit: rowsPerPage, sortBy: sortCol, sortOrder: sortDir };
  ...
  const toggleAll = () => {
    if (allPageSelected) {
      const next = new Set(selected); paginated.forEach((p) => next.delete(p.id)); setSelected(next);
    } else {
      const next = new Set(selected); paginated.forEach((p) => next.add(p.id)); setSelected(next);
    }
  };
  ...
  const handleBulkDeleteClick = () => { if (selected.size === 0) return; setShowConfirm(true); };
  ...
  const bulkDelete = async () => {
    ...
    try {
      ...
      setSelected(new Set());
      load();
    } catch { toast.error('Error al eliminar los productos'); }
  ...
  const SortArrow = ({ col }: { col: typeof sortCol }) =>
    sortCol !== col ? null : <span className="ml-0.5 text-brand-600">{sortDir === 'asc' ? '↑' : '↓'}</span>;
          ...
          {loading ? (
            ...
            return (
              <div key={p.id} className={`flex items-center gap-3 px-4 py-3 ${selected.has(p.id) ? 'bg-brand-50' : ''}`}>
                ...
                <div className="flex-1 min-w-0" onClick={() => openEdit(p.id)}>
                  <p className="font-medium text-gray-800 text-sm leading-snug truncate cursor-pointer hover:text-brand-600">{p.name}</p>
                  ...
                  {m !== null && <span className={`text-xs ${m < 20 ? 'text-amber-500' : m >= 50 ? 'text-green-600' : 'text-gray-400'}`}>Margen {m.toFixed(0)}%</span>}
                </div>
              ...
              {loading ? (
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer ${selected.has(p.id) ? 'bg-brand-50' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-800 hover:text-brand-600">{p.name}</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-1">
              <button disabled={safePage === 1} onClick={() => setPage((p) => p - 1)}
                className="px-2 py-1 rounded border border-gray-200 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">‹</button>
